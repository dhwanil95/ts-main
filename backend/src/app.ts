...
export class App {
  ...
  async process (appParameters: AppParameters): Promise<void> {
    ...
    console.log(`Counted ${cnt} instances of '${appParameters.word}' in the body of the page`);
  }
  ...
  async countWordBFS(url: string, word: string, maxDepth: number): Promise<number>{
    ...
    const temp: {url: string, depthLevel: number}[] = [{url: url, depthLevel: 0}]
    let ans = 0

    for(let i = 0; i < temp.length; i++){
      const {url, depthLevel} = temp[i]
      if(counted.has(url) || depthLevel > maxDepth) {
        continue
      }
      counted.add(url)
